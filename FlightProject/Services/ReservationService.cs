...
namespace FlightProject.Services
{
    public class ReservationService
    {
        ...
        public async Task<List<Reservation>> GetReservationsClientsAsync(string client)
        {
            return await _reservations.Find(x => x.client == client).ToListAsync();
        }
        public async Task<List<Reservation>> GetReservationsVolAsync(string vols)
        {
            return await _reservations.Find(x => x.vol == vols).ToListAsync();
        }
    }
}
